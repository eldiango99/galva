...
function computeUnitWeight(profil, longueur){
  const wpm = parseProfilWeightPerMeter(profil);
  const len = parseFloat(longueur) || 0;
  if(wpm == null || !len) return 0;
  return Math.round(wpm * len * 1000) / 1000;
}
...
function renderPieceRows(){
  ...
  wrap.innerHTML = newPieces.map((p, i) => {
    ...
    <div class="repere-row">
      ...
      <input placeholder="${t('new.longueur')}" type="number" min="0" step="0.01" value="${p.longueur||''}" data-i="${i}" data-f="longueur" class="prow">
