...
// ===================== CALCUL DU POIDS PAR PROFIL =====================
// Cornières (profilés en L) : poids ~= section(mm²) x 0.00785 kg/mm²/m (densité acier 7.85 g/cm3)
// Aile égale  "L40x3"      -> a=40mm, épaisseur=3mm      -> section ≈ t(2a - t)
// Aile inégale "L50x30x5"  -> a=50mm, b=30mm, épaisseur=5mm -> section ≈ t(a + b - t)
const PROFIL_SUGGESTIONS = ['L25x3','L30x3','L30x4','L35x4','L40x3','L40x4','L40x5','L45x4','L45x5','L50x4','L50x5','L50x6',
  'L60x5','L60x6','L65x6','L65x7','L70x6','L70x7','L75x6','L75x7','L75x8','L80x6','L80x8','L90x7','L90x8','L90x9',
  'L100x8','L100x9','L100x10','L110x10','L120x10','L120x12','L130x10','L130x12','L150x10','L150x12','L150x15',
  'L160x14','L160x16','L180x14','L180x16','L200x16','L200x18','L200x20'];

function parseProfilWeightPerMeter(profilRaw){
  if(!profilRaw) return null;
  let s = String(profilRaw).toUpperCase().replace(/\s+/g,'').replace(/×/g,'X');
  if(s[0] !== 'L') return null;
  s = s.slice(1);
  const parts = s.split('X').map(v => parseFloat(v)).filter(v => !isNaN(v) && v > 0);
  let areaMM2 = null;
  if(parts.length === 2){
    const [a,t] = parts;
    if(t >= a) return null;
  ...
  const kgPerM = areaMM2 * 0.00785; // 7.85e-6 kg/mm3 * 1000mm
  return Math.round(kgPerM * 10000) / 10000;
}

function computeUnitWeight(profil, longueurMM){
  const wpm = parseProfilWeightPerMeter(profil); // kg par mètre
  const lenMM = parseFloat(longueurMM) || 0;
  if(wpm == null || !lenMM) return 0;
  const lenM = lenMM / 1000; // conversion mm -> m
  return Math.round(wpm * lenM * 1000) / 1000;
}
...
function renderNewShipment(root){
  ...
  root.innerHTML = `
    ...
    <div class="card mb">
      ...
      <div class="notice-box notice-info">${ICONS.info}<span>${t('new.weighthint')}</span></div>
      <datalist id="profilList">${PROFIL_SUGGESTIONS.map(p=>`<option value="${p}">`).join('')}</datalist>
  ...
  `;
  document.getElementById('addRowBtn').addEventListener('click', () => { newPieces.push({repere:'', quantite:0, profil:'', longueur:0, poidsUnitaire:0}); renderPieceRows(); });
  document.getElementById('saveShipmentBtn').addEventListener('click', saveNewShipment);
  setupOcrZone('new');
  if(!newPieces.length){ newPieces.push({repere:'', quantite:0, profil:'', longueur:0, poidsUnitaire:0}); }
  renderPieceRows();
}
...
function renderPieceRows(){
  ...
  wrap.innerHTML = newPieces.map((p, i) => {
    const wpm = parseProfilWeightPerMeter(p.profil);
    const profilBad = p.profil && wpm == null;
    return `
    <div class="repere-row">
      <input placeholder="${t('new.repere')}" value="${esc(p.repere)}" data-i="${i}" data-f="repere" class="prow">
      <input placeholder="${t('new.profil')}" value="${esc(p.profil||'')}" data-i="${i}" data-f="profil" class="prow ${profilBad?'profil-bad':''}" list="profilList" title="${profilBad?t('new.profilunknown'):''}">
      <input placeholder="${t('new.longueur')}" type="number" min="0" step="1" value="${p.longueur||''}" data-i="${i}" data-f="longueur" class="prow">
      <input placeholder="${t('new.qty')}" type="number" min="0" value="${p.quantite||''}" data-i="${i}" data-f="quantite" class="prow">
      <input readonly value="${fmt(p.poidsUnitaire||0)}" tabindex="-1" title="${t('new.punit')}">
      <input readonly value="${fmt((p.quantite||0)*(p.poidsUnitaire||0))}" tabindex="-1" title="${t('new.ptotal')}">
  ...
  wrap.querySelectorAll('.prow').forEach(inp => {
    inp.addEventListener('input', (e) => {
      ...
      const item = newPieces[i];
      item[f] = (f==='repere' || f==='profil') ? e.target.value : parseFloat(e.target.value)||0;
      item.poidsUnitaire = computeUnitWeight(item.profil, item.longueur);
      updateGauge();
      // refresh computed columns + profil validity without full rerender to keep focus
      const row = e.target.closest('.repere-row');
      row.children[4].value = fmt(item.poidsUnitaire);
      row.children[5].value = fmt((item.quantite||0)*(item.poidsUnitaire||0));
      const profilInp = row.children[1];
      const wpm = parseProfilWeightPerMeter(item.profil);
      const bad = item.profil && wpm == null;
      profilInp.classList.toggle('profil-bad', !!bad);
      profilInp.title = bad ? t('new.profilunknown') : '';
    });
  });
  wrap.querySelectorAll('.rm-btn').forEach(btn => {
    btn.addEventListener('click', (e) => { newPieces.splice(parseInt(e.currentTarget.dataset.i),1); if(!newPieces.length) newPieces.push({repere:'',quantite:0,profil:'',longueur:0,poidsUnitaire:0}); renderPieceRows(); });
  });
  updateGauge();
}
...
async function saveNewShipment(){
  ...
  const exp = {
    ...
    pieces: validPieces.map(p => ({repere:p.repere.trim().toUpperCase(), quantite:p.quantite||0, profil:(p.profil||'').trim().toUpperCase(), longueur:p.longueur||0, poidsUnitaire:p.poidsUnitaire||0})),
...
function addOcrCode(code, target){
  if(target === 'new'){
    ...
    if(existing){ existing.quantite = (existing.quantite||0) + 1; }
      ...
      else newPieces.push({repere: code, quantite: 1, profil:'', longueur:0, poidsUnitaire: 0});
    }
    renderPieceRows();
